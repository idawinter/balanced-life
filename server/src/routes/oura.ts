...
ouraRouter.get("/connect", async (req: Request, res: Response) => {
  ...
  authUrl.searchParams.set("scope", "email personal daily"); // minimal example scopes
  authUrl.searchParams.set("state", userId); // we’ll echo this back to know which user
 ...
 * Exchange code -> token; persist token for the user.
 */
ouraRouter.get("/callback", async (req: Request, res: Response) => {
  try {
    ...
    // Simple “done” page:
    return res.send(
      `<html><body><h1>Oura connected ✅</h1><p>You can close this tab and return to the app.</p></body></html>`
    );
  } catch (e: any) {
 ...
 * Example: fetch recent sleep summary using the stored access token.
 */
ouraRouter.get("/pull", async (req: Request, res: Response) => {
  try {
    ...
    // Example: fetch sleep summary (adjust endpoints/scopes as needed)
